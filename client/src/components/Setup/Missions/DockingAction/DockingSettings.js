...
import './DockingSettings.css';

const DockingSettings = ({ action, onClose, points, markers, onSave }) => {
  const { updateAction } = useSettings();
  
  // Sử dụng cấu trúc field configs có sẵn
  const fieldConfigs = createActionFieldConfigs.docking(markers); // markers is now mapWithMarkers
  ...
  function findMarkerNameById_Functional(mapWithMarkers, targetId) {
    // Tìm marker theo ID trong mapWithMarkers
    for (const mapGroup of mapWithMarkers) {
      const foundMarker = mapGroup.markers.find(marker => marker.ID === targetId);
      if (foundMarker) {
        return foundMarker.MarkerName;
      }
    }
    return null;
  }
  ...
  const handleValidate = () => {
    try {
      ...
      let markerName; // Name of the point to dock to

      if(getAction(actionData).marker.userVariable =='false') {
        markerName = findMarkerNameById_Functional(markers, parseInt(getAction(actionData).marker.variable));
      } else {
        markerName = '';
      }
      ...
      console.error('Error updating Docking action:', error);
    }
  };
